...
function JADE(FUN, DIM, ftarget, maxfunevals)
    ...
    crossover_op = 'JA';   % type of crossover operator: ['bin' or 'exp' or 'JA']
    mutation_op = 'JA';   % type of mutation operator: ['rand' or 'best' or 'average' or 'JA"]
    
    %F = 0.8;      % set parameter
    CR = 0.5;      % set cross-over probability
    
    useAE = false;       % To use DE: set false, to use DE+AE: set true
    ...
    % init JA-mutation
    adaptJA_muF = true;    % use adaptation of mu_F
    adaptJA_muCR = true;  % use adaptation of mu_CR
    ...
    if useAE
        AE = AdaptiveEncoding(DIM, popsize, eval('ceil(popsize/2)'), pop_de);
        AE.c_1 = eval('0.1');
        AE.c_mu = eval('0.1');
        AE.c_p = eval('1/sqrt(AE.DIM)');
    end
